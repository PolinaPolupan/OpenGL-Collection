...
struct Material
{
	...
	void SetDiffuseMap(std::string path) {
		diffuseMap = std::make_unique<Texture>(path, Texture::Type::Diffuse);
		diffuseMap->Bind(0);
		diffuseColor = glm::vec3(1.f, 1.f, 1.f);
	}
	...
	void SetSpecularMap(std::string path) {
		specularMap = std::make_unique<Texture>(path, Texture::Type::Specular);
		specularMap->Bind(1);
		specularColor = glm::vec3(1.f, 1.f, 1.f);
	}
	...
	void SetEmissionMap(std::string path) {
		emissionMap = std::make_unique<Texture>(path, Texture::Type::Emission);
		emissionMap->Bind(2);
	}
